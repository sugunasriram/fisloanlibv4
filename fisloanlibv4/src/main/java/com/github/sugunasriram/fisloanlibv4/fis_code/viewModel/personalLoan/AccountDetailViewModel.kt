...
class AccountDetailViewModel : BaseViewModel() {
    ...
    fun onAccountHolderChanged(accountHolder: String) {
        ...
        if (accountHolder.length <= 35) {
            _accountHolder.value = sanitizedInput
    ...
    fun onIfscCodeChanged(ifscCode: String) {
        ...
        if (sanitizedInput.length <= 11) {
            _ifscCode.value = sanitizedInput.take(11)
            updateGeneralError(null)
        }
    }
    ...
    fun bankAccountDetailValidation(
        ...
        if (!Pattern.compile("^[a-zA-Z ]*$").matcher(accountHolder).find()) {
        ...
        if (!Pattern.compile("^\\d+$").matcher(accountNumber).find()) {
        ...
        if (CommonMethods().isValidIfscCode(ifscCode) != true) {
