...
class PersonalLoanViewModel : BaseViewModel() {
    ...
    private val _pLSearchResponse = MutableStateFlow<LoanSearchResponse?>(null)
    val pLSearchResponse: StateFlow<LoanSearchResponse?> = _pLSearchResponse

    fun personalLoanSearch(context: Context, searchBodyModel: SearchBodyModel) {
        _searchInProgress.value = true
        viewModelScope.launch(Dispatchers.IO) {
            handlePLSearchApi(context, searchBodyModel)
        }
    }

    private suspend fun handlePLSearchApi(
        context: Context,
        searchBodyModel: SearchBodyModel,
        checkForAccessToken: Boolean = true
    ) {
        kotlin.runCatching {
            ApiRepository.pLSearchApi(searchBodyModel)
        }.onSuccess { response ->
            response?.let {
                Log.d("res_H-search", response.toString())
                handleSearchApiSuccess(response)
            }
        }.onFailure { error ->
            Log.d("res_H-search", error.toString())
            if (checkForAccessToken &&
                ...
                if (handleAuthGetAccessTokenApi()) {
                    handlePLSearchApi(context, searchBodyModel, false)
                } else {
    ...
    private suspend fun handleSearchApiSuccess(response: LoanSearchResponse) {
        withContext(Dispatchers.Main) {
            ...
            _pLSearchResponse.value = response
        }
    }
