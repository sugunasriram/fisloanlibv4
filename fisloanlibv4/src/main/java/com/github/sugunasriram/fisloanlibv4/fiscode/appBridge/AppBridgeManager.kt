        ...
        when {
            ...
            unAuthorizedUser -> CommonMethods().ShowUnAuthorizedErrorScreen(navController = rememberNavController())
                ...
                sessionDetails?.sessionId?.let { sessionId ->
                    ...
                    if (isVerifySessionChecking ) {
                        Log.d("AppBridgeManager", "VerifySession in Progress")

                        CenterProgress()

                    } else if (isVerifySessionSuccess) {
                    Log.d("AppBridgeManager", "VerifySession Done")

//                        val route = AppScreens.DownPaymentScreen.route
//                        navController.currentBackStackEntry?.savedStateHandle?.set("verifySessionResponse", verifySessionResponse)
//                        LaunchScreen(route)
...
//                        LaunchScreen(AppScreens.DownPaymentScreen.route)
//                        val route = AppScreens.DownPaymentScreen.route
